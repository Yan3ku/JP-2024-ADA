...
procedure Simulation is
   ...
   Assembly_Name: constant array (Assembly_Type) of String(1 .. 11)
     := ("Zamowienie1", "Zamowienie2", "Zamowienie3");
   ...
   begin
      ...
      Put_Line(ESC & "[93m" & "K: Zaczeto serwowac " & Product_Name(Producer_Type_Number) & ESC & "[0m");
      loop
         Random_Time := Duration(Random_Production.Random(G));
         delay Random_Time;
         Put_Line(ESC & "[93m" & "K: Ugotowano " & Product_Name(Producer_Type_Number)
   ...
   begin
      ...
      Put_Line(ESC & "[96m" & "K: " & Consumer_Name(Consumer_Nb) & " zaczol prace" & ESC & "[0m");
      loop
         delay Duration(Random_Consumption.Random(G)); --  simulate consumption
         Assembly_Type := Random_Assembly.Random(GA);
         -- take an assembly for consumption
         B.Deliver(Assembly_Type, Assembly_Number);
         Put_Line(ESC & "[96m" & "K: " & Consumer_Name(Consumer_Nb) & " bierze zamowienie " &
   ...
   --Buffer/Kuchnia--
   -- assembly to zamowienie ktory sklada sie z zestawu dań --
   ...
   task body Buffer is
      ...
      begin
         for W in Producer_Type loop
            Put_Line("|   Dania oczekujace dostarczenia: " & Integer'Image(Storage(W)) & " "
                     & Product_Name(W));
         end loop;
         Put_Line("|   Razem: " & Integer'Image(In_Storage));

      end Storage_Contents;

   begin
      Put_Line(ESC & "[91m" & "K: Buffer started" & ESC & "[0m");
      Setup_Variables;
      loop
      	select
	      accept Take(Product: in Producer_Type; Number: in Integer) do
		 if Can_Accept(Product) then
		    Put_Line(ESC & "[91m" & "K: " & Product_Name(Product) & " numer " &
			       Integer'Image(Number) & " czeka na dostarczenie"  & ESC & "[0m");
		    Storage(Product) := Storage(Product) + 1;
		    In_Storage := In_Storage + 1;
		 else
		    Put_Line(ESC & "[91m" & "K: Kuchnia nie ma miejsca na " & Product_Name(Product) & " number " &
	...
	or
		...
		if Can_Deliver(Assembly) then
		   Put_Line(ESC & "[91m" & "K: Kuchnia przyjela zamowienie " & Assembly_Name(Assembly) & " numer " &
		   ...
		   Put_Line(ESC & "[91m" & "B: Kuchnia nie moze zrealizowac " & Assembly_Name(Assembly)& ESC & "[0m");
		   Number := 0;
		end if;
	     end Deliver;
	     Storage_Contents;
